...
import { additionWorksheets } from "../Data/data";

const AdditionWorksheets = () => {
  return (
    <>
      <Header />
                  ...
                  <div className="w-full grid md:gap-10 gap-5 lg:pr-10 sm:grid-cols-4 grid-cols-2">
                    {sheet.sheets?.map((item) => (
                      <Link to={item.url} key={item.url} className="w-full">
                        <img src={item.img} alt="" className="w-full transition-all duration-300 hover:scale-105" />
                      </Link>
                    ))}
                  </div>
                </div>
...
export default AdditionWorksheets;
